...
namespace LaserGRBL.PSHelper
{
    public partial class PSEditorForm : Form
    {
        ...
        {
            if (e.ColumnIndex == ColPower.Index || e.ColumnIndex == ColSpeed.Index || e.ColumnIndex == ColPowerY.Index || e.ColumnIndex == ColSpeedY.Index)
            {
                try
                {
                ...
                if (o.GetDataPresent(DataFormats.Text))
                {
                    ...
                    foreach (string row in rows)
                    {
                        try
                        {
                            ...
                            GrblCore.MaterialDB.Materials.AddMaterialsRow(Guid.NewGuid(), true, cells[0], cells[1], cells[3], cells[2], GetInt(cells[4]), GetInt(cells[5]), GetInt(cells[6]), cells[7], GetInt(cells[8]), GetInt(cells[9]));
                        }
