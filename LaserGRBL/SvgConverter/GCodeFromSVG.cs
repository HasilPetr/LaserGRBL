...
namespace LaserGRBL.SvgConverter
{
    ...
    class GCodeFromSVG
    {
        ...
        private bool filterByColor(XElement pathElement, ColorFilter filter)
        {
            ...
            if (styleMap?.TryGetValue(SvgStrokeAttribute, out var pathColor) != true)
                if (styleMap?.TryGetValue(SvgFillAttribute, out pathColor) != true)
                    if (colorMap?.TryGetValue(SvgStrokeAttribute, out pathColor) != true)
                        if (colorMap?.TryGetValue(SvgFillAttribute, out pathColor) != true)
                            return true;
