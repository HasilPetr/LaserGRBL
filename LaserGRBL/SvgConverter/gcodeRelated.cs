...
namespace LaserGRBL.SvgConverter
{
    public static class gcode
    {
        ...
        private static float gcodeXFeed = 1999;        // X feed to apply for G1
        ...
        public static void setup(GrblCore core)
        {
            ...
            gcodeXFeed = Settings.GetObject("GrayScaleConversion.VectorizeOptions.XSpeed", 1000);
            gcodeYFeed = Settings.GetObject("GrayScaleConversion.VectorizeOptions.YSpeed", 1000);
        ...
        private static void Move(StringBuilder gcodeString, int gnr, float x, float y, float? z, bool applyFeed, string cmt)
        {
            ...
            if (gcodeCompress)
            {
                if (((gnr > 0) || (lastx != x) || (lasty != y) || (lastz != tz)))  // else nothing to do
                {
                    ...
                    if ((gnr == 1) && (lastf != xyFeed) || applyFeed)
                    {
                        gcodeTmp.AppendFormat("F{0}", xyFeed);
                        lastf = xyFeed;
                        isneeded = true;
                    }
